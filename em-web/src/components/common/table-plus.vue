<template>
  <div>
    <el-table :data="tableData" :stripe="false" :border='false' :fit="true" :show-header="true"
    ...
    <template v-slot="{row,$index}">    
        ...
        <!-- 图片展示 -->   
            ...
            <template v-slot:reference>
						<el-image v-if="column.image" :src="viewUrl + row[column.prop]"></el-image>
            </template>
					</el-popover>
        <!-- 可编辑input，仅在text默认展示类型才可编辑-->
            ...
            <template v-for="(operations, index) in column.operation" :key="index"> 
    ...
    <!-- 分页 -->
		<div class="page_div" :style="{textAlign: pageObj.position || 'center'}">
			<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
				...
				:page-sizes="[10,50,100,200,400]" :page-size="pageObj.pageData.size" background
